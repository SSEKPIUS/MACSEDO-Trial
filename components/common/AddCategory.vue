<template>
  <div>
    <div class="flex items-center max-w-md mx-auto bg-white rounded-full border drop-shadow-lg ">
      ...
      <div>
        <button type="submit" class="flex items-center justify-center w-12 h-12 text-gray-100 rounded-full m-1" :class="(search.length > 0) ? 'bg-purple-500' :'bg-gray-500 cursor-not-allowed'" :disabled="search.length == 0 || loading" @click="fireEvent">
...
export default {
  ...
  methods: {
    fireEvent () {
      this.$emit('firedEvent', this.search)
    }
  }
}
</script>
