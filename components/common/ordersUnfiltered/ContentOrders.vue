<template>
  <div>
    <div class="  w-full p-2 h-full">
      <div class=" w-full p-2  shadow-2xl rounded">
        <div class="p-1 rounded-l-md h-8 w-min rounded" :class="getLoadingOrders ? '' : 'bg-gray-200'">
          <svg
      ...
      <!--pagination-->
      <div v-show="getOrdersPaginated.links" class=" w-full p-2 shadow-2xl rounded mt-2  ">
        <div class=" m-2 flex flex-row flex-nowrap justify-center items-center">
          <!--eslint-disable-next-line vue/no-v-html-->
          <!--eslint-disable-next-line vue/max-attributes-per-line-->
          <span v-for="(link, n) in getOrdersPaginated.links" :key="n" :class="getOrdersPaginated.current_page == link.label ? ' text-red-600' : ' text-black'" class=" px-2 cursor-pointer" @click="loadPaginationPage(link.url)" v-html="link.label" />
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['getOrdersPaginated', 'getLoadingOrders', 'getSearch'])
  },
