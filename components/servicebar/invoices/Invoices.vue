  ...
  <div class="h-auto">
    ...
    <div class=" flex justify-center items-center  w-full">
      <ul class="flex justify-center items-center my-2">
        <li class="cursor-pointer py-2 px-2" @click="setActiveTab(0)">
          <button :class="getActiveTab==0 ? 'bg-blue-600' : 'bg-blue-400'" class="relative text-white px-6 py-2 rounded text-base font-bold overflow-visible">
            SERVICE-BAR
            <div v-show="getserviceBarArrCount>0" class="absolute top-0 right-0 -mt-5 -mr-1 px-4 py-1 bg-red-500 rounded-full">
              {{ getserviceBarArrCount }}
            </div>
          </button>
        </li>
        <li class="cursor-pointer py-2 px-2" @click="setActiveTab(1)">
          <button :class="getActiveTab==1 ? 'bg-blue-600' : 'bg-blue-400'" class="relative text-white px-6 py-2 rounded text-base font-bold overflow-visible">
            SERVICE-WAITER-WAITRESS
            <div v-show="getserviceWaiterWaitressArrCount>0" class="absolute top-0 right-0 -mt-5 -mr-1 px-4 py-1 bg-red-500 rounded-full">
              {{ getserviceWaiterWaitressArrCount }}
            </div>
          </button>
        </li>
        <li class="cursor-pointer py-2 px-2" @click="setActiveTab(2)">
          <button :class="getActiveTab==2 ? 'bg-blue-600' : 'bg-blue-400'" class="relative text-white px-6 py-2 rounded text-base font-bold overflow-visible">
            RECEPTION
            <div v-show="getreceptionArrCount>0" class="absolute top-0 right-0 -mt-5 -mr-1 px-4 py-1 bg-red-500 rounded-full">
              {{ getreceptionArrCount }}
            </div>
          </button>
        </li>
        <li class="cursor-pointer py-2 px-2" @click="setActiveTab(3)">
          <button :class="getActiveTab==3 ? 'bg-blue-600' : 'bg-blue-400'" class="relative text-white px-6 py-2 rounded text-base font-bold overflow-visible">
            STEAM-SAUNA-MASSAGE
            <div v-show="steamSaunaMassageArrCount>0" class="absolute top-0 right-0 -mt-5 -mr-1 px-4 py-1 bg-red-500 rounded-full">
...
// 'SUPERVISOR', 'SERVICE-BAR', SERVICE-WAITER-WAITRESS, 'RECEPTION', 'KITCHEN', 'STEAM-SAUNA-MASSAGE', 'STORE', 'ACCOUNTS'
...
export default {
  ...
  watch: {
    receipts: {
      handler (newVal) {
        this.serviceBarArr = _.filter(newVal, function (o) {
        ...
        this.receptionArr = _.filter(newVal, function (o) {
          return (o.section == 'RECEPTION')
        })

        this.steamSaunaMassageArr = _.filter(newVal, function (o) {
          return (o.section == 'STEAM-SAUNA-MASSAGE')
        })
      }
    }
  },
