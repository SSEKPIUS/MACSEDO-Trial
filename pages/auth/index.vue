<template>
  <section class=" w-full h-screen overflow-hidden bg-auth-pattern bg-no-repeat bg-cover flex flex-row md:flex-col items-center justify-center">
    <div class="w-3/12 md:w-10/12 top-40 md:top-20  rounded overflow-hidden shadow-lg bg-gray-50 m-1">
      <div class=" p-5">
        <p class="p-4 w-full text-center uppercase font-black">
        ...
        <form method="post" @submit.prevent="login">
          ...
          <button v-if="!loading" type="submit" class="bg-black hover:bg-blue-800 w-full rounded-lg text-indigo-50 pt-2 pb-2  uppercase">
            Log In
          </button>
...
export default {
  ...
  data () {
    return {
      loading: false,
      error: null,
      // accounts: ['SUPERVISOR', 'SERVICE-BAR', 'SERVICE-WAITER-WAITRESS', 'RECEPTION', 'KITCHEN', 'STEAM-SAUNA-MASSAGE', 'STORE', 'ACCOUNTS'],
      mailhasError: false,
      pwhasError: false,
      form: {
